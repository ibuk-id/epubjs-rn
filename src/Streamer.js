...
class EpubStreamer {
  ...
  open(filePath, id) {
    const rootDir = Dirs.DocumentDir + '/' + this.root;
    this.unzipPath = Dirs.DocumentDir + '/' + this.root + '/' + id + 'x';
    // Add the directory
    return RNFetchBlob.fs.exists(`${Dirs.DocumentDir}/${this.root}`)
      ...
      .then(() => {
        return unzip(filePath, unzipPath).then((newPath) => {
          return newPath;
        });
      })
      .then(() => {
  ...
  kill() {
    ...
    if (this.unzipPath) {
      RNFetchBlob.fs.unlink(this.unzipPath);
    }
  }
